...
# ✅ Set memory limit for node processes (prevents heap crash)
...
RUN if [ "$USE_OLLAMA" = "true" ]; then \
    ...
    apt-get update && \
    apt-get install -y --no-install-recommends \
        git build-essential pandoc gcc netcat-openbsd \
...
# Install Python dependencies and preload models
RUN pip3 install uv && \
...
# Copy frontend build from earlier stage
